...
func extractFromCtx(ctx context.Context) (receivingAccount, requestingAccount *gtsmodel.Account) {
	...
	if receivingAccountI != nil {
		...
		if !ok {
			log.Panicf(ctx, "extractFromCtx: context entry with key %s could not be asserted to *gtsmodel.Account", ap.ContextReceivingAccount)
		}
	}
	...
	if requestingAcctI != nil {
		...
		if !ok {
			log.Panicf(ctx, "extractFromCtx: context entry with key %s could not be asserted to *gtsmodel.Account", ap.ContextRequestingAccount)
		}
	}
